...
def run_unit_tests():
    ...
    """
    tests = os.path.join('exepiest', 'tests')
...
def run_copyright_checks():
    ...
    # Recursively walk the exepiest directory and check copyright header is in
...
#"""
    ...
    for dirname, subdir_list, file_list in os.walk('exepiest'):
...
def doctest_rst_and_public_interface():
    ...
    # Import all public exepiest modules. We list
    # everything here for completeness. If a new module is added to exepiest
    ...
    # has been added to exepiest and should be imported above and included in
    # this list.
    exepiest_submodules = [
        'exepiest.models',
        'exepiest.new_models',
        'exepiest.version_info',
        'exepiest.simulation',
        'exepiest.apps',
        'exepiest.posterior',
        'exepiest.new_posterior',
        'exepiest.figures'
        ]

    doc_symbols = get_all_documented_symbols()

    check_exposed_symbols(epiestim, exepiest_submodules, doc_symbols)
...
def check_exposed_symbols(module, submodule_names, doc_symbols):
    ...
    if len(unexpected_modules) > 0:
        ...
        print('For python modules such as numpy you may need to confine the '
              ...
              'exepiest submodule, you will need to make %s (doctest) aware'
              'of this.'
              % __file__)
        print('FAILED')
        sys.exit(1)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Run unit test for exepiest',
        epilog='To run individual unit tests, use e.g.'
               ' $ python3 exepiest/tests/test_dummy.py',
    )
    # Unit tests
    parser.add_argument(
        '--unit',
